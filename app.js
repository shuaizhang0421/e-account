(function () {
  ...
  function repaymentFields(account) { const target = accountById(draft.targetAccountId); const statements = target ? creditStatements(target.id, draft.targetCurrency, { excludeTransactionId: editing }).filter((item) => item.remaining > 0) : []; if (target && !statements.length && draft.paymentMode !== "advance") { draft.paymentMode = "advance"; draft.statementAllocations = []; } return `${accountPicker("付款账户", orderedAccounts().filter((a) => a.type !== "credit"), draft.accountId, "account")}${currencyPicker("付款币种", account?.currencies || [], draft.currency, "currency")}${amountDateFields("付款金额", "还款日期")}${accountPicker("还到信用卡", orderedAccounts().filter((a) => a.type === "credit"), draft.targetAccountId, "credit-target")}${currencyPicker("偿还币种", target?.currencies || [], draft.targetCurrency, "target-currency")}<label class="field"><span>信用卡实际还款金额</span><input id="draft-target-amount" type="number" step="0.01" min="0.01" value="${escapeAttr(draft.targetAmount || draft.amount)}" placeholder="外币还款可与付款金额不同"></label>${repaymentModePicker(statements)}${repaymentAllocationFields(target, statements)}${rateHint()}${feeFields(account)}${noteAndActions("保存还款")}`; }
  function repaymentModePicker(statements) { const statementActive = draft.paymentMode !== "advance"; return `<div class="repayment-mode-picker"><button class="${statementActive ? "active" : ""}" data-action="set-payment-mode" data-mode="statement" type="button" ${statements.length ? "" : "disabled"}>账单还款</button><button class="${!statementActive ? "active" : ""}" data-action="set-payment-mode" data-mode="advance" type="button">提前还款</button></div>`; }
  function repaymentAllocationFields(target, statements = []) {
    ...
    if (draft.paymentMode === "advance") { const outstanding = creditOutstanding(target.id, draft.targetCurrency, editing); return `<div class="repayment-allocation advance-repayment"><strong>提前还款</strong><span>本次不分配到已出账账单，还款会先进入预还款池，${draft.targetCurrency} 账单生成后自动抵扣最早账单。</span><small>当前 ${draft.targetCurrency} 欠款：${money(outstanding, draft.targetCurrency)}</small></div>`; }
  ...
  function setPaymentMode(mode) { syncDraftFromInputs(); if (!["statement", "advance"].includes(mode)) return; draft.paymentMode = mode; if (mode === "advance") draft.statementAllocations = []; render(); }
  ...
  function saveTransaction() {
    ...
    if (draft.type === "transfer" || draft.type === "exchange") { const target = accountById(draft.targetAccountId); const targetAmount = positive(draft.targetAmount || draft.amount); if (!target || !targetAmount || !target.currencies.includes(draft.targetCurrency)) return notify("请选择目标账户、币种并填写实际金额。"); if (draft.type === "transfer" && (target.type !== "credit" || source.type === "credit")) return notify("请使用非信用卡账户归还信用卡。"); if (draft.type === "exchange" && target.type === "credit") return notify("购汇入账账户不能是信用卡。"); item.targetAccountId = target.id; item.targetAmount = targetAmount; item.targetCurrency = draft.targetCurrency; item.exchangeRate = round(amount / targetAmount); item.feeAmount = Math.max(0, round(draft.feeAmount)); item.feeCurrency = item.feeAmount ? (source.currencies.includes(draft.feeCurrency) ? draft.feeCurrency : item.currency) : ""; if (draft.type === "transfer") { item.paymentMode = draft.paymentMode === "advance" ? "advance" : "statement"; if (item.paymentMode === "advance") { const outstanding = creditOutstanding(target.id, item.targetCurrency, editing); if (targetAmount > outstanding + 0.009) return notify(`提前还款不能超过当前 ${item.targetCurrency} 欠款 ${money(outstanding, item.targetCurrency)}。`); item.statementAllocations = []; } else { const available = creditStatements(target.id, item.targetCurrency, { excludeTransactionId: editing }); const byId = new Map(available.map((statement) => [statement.id, statement])); const allocations = (draft.statementAllocations || []).map((allocation) => ({ statementId: allocation.statementId, amount: Math.max(0, round(allocation.amount)) })).filter((allocation) => allocation.amount > 0); const allocated = round(allocations.reduce((sum, allocation) => sum + allocation.amount, 0)); if (!allocations.length || Math.abs(allocated - targetAmount) > 0.009) return notify("请勾选账单，并使分配合计等于实际还款金额。"); if (allocations.some((allocation) => !byId.has(allocation.statementId) || allocation.amount > byId.get(allocation.statementId).remaining + 0.009)) return notify("还款分配超过了所选账单的剩余应还金额。"); item.statementAllocations = allocations; } } }
  ...
  function creditPaymentMode(transaction) { const target = accountById(transaction.targetAccountId); if (transaction.type !== "transfer" || target?.type !== "credit") return "statement"; if (transaction.paymentMode === "advance" || (!transaction.paymentMode && !(transaction.statementAllocations || []).length)) return "advance"; return "statement"; }
  ...
  function creditStatements(accountId = "", currency = "", options = {}) {
    ...
    const paid = {}; const advancePayments = [];
    state.transactions.filter((transaction) => transaction.type === "transfer" && transaction.id !== excludedTransactionId).forEach((transaction) => {
      ...
      if (creditPaymentMode(transaction) === "advance") advancePayments.push(transaction);
    });
    ...
    const automaticPaid = {};
    advancePayments.sort((a, b) => a.date.localeCompare(b.date)).forEach((transaction) => {
      let remaining = Math.max(0, round(transaction.targetAmount));
      orderedStatements.filter((statement) => statement.accountId === transaction.targetAccountId && statement.currency === transaction.targetCurrency && statement.statementDate >= transaction.date).forEach((statement) => {
        if (!remaining) return;
  ...
  function startCreditPayment(accountId, currency, statementId = "") { const source = orderedAccounts().find((account) => account.type !== "credit"); const target = accountById(accountId); if (!source || !target) return notify("请先添加付款账户和信用卡。"); const statements = creditStatements(target.id, currency).filter((item) => item.remaining > 0); const chosen = statementId ? statements.filter((item) => item.id === statementId) : statements; if (statementId && !chosen.length) return notify("这张账单已经没有剩余应还。"); if (!chosen.length && creditOutstanding(target.id, currency) <= 0) return notify(`当前 ${currency} 没有可提前偿还的欠款。`); draft = newDraft("transfer"); draft.accountId = source.id; draft.targetAccountId = target.id; draft.paymentMode = chosen.length ? "statement" : "advance"; pickerTypes.account = source.type; pickerTypes["credit-target"] = "credit"; draft.targetCurrency = currency; draft.currency = source.currencies.includes(currency) ? currency : source.currencies[0] || BASE_CURRENCY; draft.statementAllocations = chosen.map((item) => ({ statementId: item.id, amount: item.remaining })); draft.targetAmount = chosen.length ? round(draft.statementAllocations.reduce((sum, item) => sum + item.amount, 0)) : ""; draft.amount = draft.currency === currency && chosen.length ? draft.targetAmount : ""; view = "record"; render(); }
