(function () {
  ...
  function repaymentFields(account) {
    const target = accountById(draft.targetAccountId);
    const statements = target ? creditStatements(target.id, draft.targetCurrency, { excludeTransactionId: editing }).filter((item) => item.remaining > 0) : [];
    const crossCurrency = Boolean(account && target && draft.currency !== draft.targetCurrency);
    if (target && crossCurrency && !editing && !draft.fullBalanceOptOut) draft.paymentMode = "full-balance";
    if (target && crossCurrency && draft.paymentMode === "full-balance") draft.targetAmount = round(creditOutstanding(target.id, draft.targetCurrency, editing));
    if (target && crossCurrency && !["full-balance", "statement", "advance"].includes(draft.paymentMode)) draft.paymentMode = statements.length ? "statement" : "advance";
    if (target && !crossCurrency && draft.paymentMode === "full-balance") draft.paymentMode = statements.length ? "statement" : "advance";
    if (target && !statements.length && draft.paymentMode === "statement") { draft.paymentMode = "advance"; draft.statementAllocations = []; }
    const targetAmount = draft.paymentMode === "full-balance" && target ? creditOutstanding(target.id, draft.targetCurrency, editing) : draft.targetAmount;
    const targetReadOnly = draft.paymentMode === "full-balance" ? "readonly" : "";
    const targetLabel = draft.paymentMode === "full-balance" ? "全额抵扣金额" : "信用卡实际还款金额";
    const targetHint = draft.paymentMode === "full-balance" ? "将当前该币种欠款全部抵扣，人民币付款金额按实际扣款填写" : "外币还款可手动填写实际抵扣金额";
    return `${accountPicker("付款账户", orderedAccounts().filter((a) => a.type !== "credit"), draft.accountId, "account")}${currencyPicker("付款币种", account?.currencies || [], draft.currency, "currency")}${amountDateFields("付款金额", "还款日期")}${accountPicker("还到信用卡", orderedAccounts().filter((a) => a.type === "credit"), draft.targetAccountId, "credit-target")}${currencyPicker("偿还币种", target?.currencies || [], draft.targetCurrency, "target-currency")}<label class="field"><span>${targetLabel}</span><input id="draft-target-amount" type="number" step="0.01" min="0.01" value="${escapeAttr(targetAmount || "")}" placeholder="${targetHint}" ${targetReadOnly}></label>${repaymentModePicker(statements, account, target)}${repaymentAllocationFields(target, statements)}${rateHint()}${feeFields(account)}${noteAndActions("保存还款")}`;
  }
  function repaymentModePicker(statements, account, target) {
    const crossCurrency = Boolean(account && target && draft.currency !== draft.targetCurrency);
    if (crossCurrency) {
      const fullActive = draft.paymentMode === "full-balance";
      return `<div class="repayment-mode-picker"><button class="${fullActive ? "active" : ""}" data-action="set-payment-mode" data-mode="full-balance" type="button">按当前欠款全额抵扣</button><button class="${!fullActive ? "active" : ""}" data-action="set-payment-mode" data-mode="manual" type="button">手动抵扣金额</button></div>`;
    }
    const statementActive = draft.paymentMode !== "advance";
    return `<div class="repayment-mode-picker"><button class="${statementActive ? "active" : ""}" data-action="set-payment-mode" data-mode="statement" type="button" ${statements.length ? "" : "disabled"}>账单还款</button><button class="${!statementActive ? "active" : ""}" data-action="set-payment-mode" data-mode="advance" type="button">提前还款</button></div>`;
  }
  function repaymentAllocationFields(target, statements = []) {
    if (!target || target.type !== "credit") return empty("选择信用卡和偿还币种后，可分配到已出账账单。");
    statements = statements.length ? statements : creditStatements(target.id, draft.targetCurrency, { excludeTransactionId: editing }).filter((item) => item.remaining > 0);
    if (draft.paymentMode === "full-balance") {
      const outstanding = creditOutstanding(target.id, draft.targetCurrency, editing);
      return `<div class="repayment-allocation full-balance-repayment"><strong>跨币种全额抵扣</strong><span>本次会把当前 ${draft.targetCurrency} 欠款全部记为已还；已有账单优先分配，未出账部分会在后续账单生成后自动抵扣。</span><small>当前 ${draft.targetCurrency} 欠款：${money(outstanding, draft.targetCurrency)}。人民币付款金额按银行实际扣款填写。</small></div>`;
    }
    if (draft.paymentMode === "advance") {
      const outstanding = creditOutstanding(target.id, draft.targetCurrency, editing);
      return `<div class="repayment-allocation advance-repayment"><strong>提前还款</strong><span>本次不分配到已出账账单，还款会先进入预还款池，${draft.targetCurrency} 账单生成后自动抵扣最早账单。</span><small>当前 ${draft.targetCurrency} 欠款：${money(outstanding, draft.targetCurrency)}</small></div>`;
    }
  ...
  function rateHintText() { const rate = Number(draft.amount) > 0 && Number(draft.targetAmount) > 0 ? round(Number(draft.amount) / Number(draft.targetAmount)) : 0; const label = draft.paymentMode === "full-balance" ? "隐含汇率（仅本地记录）" : "实际汇率"; const hint = draft.paymentMode === "full-balance" ? "填写实际付款金额后，将记录本次隐含汇率。" : "填写两边金额后，将保存本次实际成交汇率。"; return rate ? `${label}：1 ${draft.targetCurrency} = ${rate} ${draft.currency}` : hint; }
  function rateHint() { return `<div class="exchange-hint">${rateHintText()}</div>`; }
  function refreshRateHint() { const hint = app.querySelector(".exchange-hint"); if (hint) hint.textContent = rateHintText(); }
  ...
  function bindStaticEvents() {
    ...
    ["draft-amount", "draft-target-amount"].forEach((id) => document.getElementById(id)?.addEventListener("input", () => { syncDraftFromInputs(); refreshRateHint(); }));
  ...
  function setPaymentMode(mode) {
    ...
    const account = accountById(draft.accountId);
    const crossCurrency = Boolean(account && target && draft.currency !== draft.targetCurrency);
    if (mode === "full-balance" && crossCurrency) {
      draft.paymentMode = "full-balance";
      draft.fullBalanceOptOut = false;
      draft.targetAmount = round(creditOutstanding(target.id, draft.targetCurrency, editing));
      draft.statementAllocations = [];
    } else if (mode === "manual" && crossCurrency) {
      ...
      draft.paymentMode = statements.length ? "statement" : "advance";
      draft.fullBalanceOptOut = true;
      draft.targetAmount = "";
      draft.statementAllocations = [];
    } else if (["statement", "advance"].includes(mode)) {
      draft.paymentMode = mode;
      if (mode === "advance") draft.statementAllocations = [];
    } else return;
    render();
  }
  ...
  function saveTransaction() {
    ...
    if (draft.type === "transfer" || draft.type === "exchange") {
      const target = accountById(draft.targetAccountId);
      const crossCurrency = draft.currency !== draft.targetCurrency;
      const requestedMode = draft.paymentMode === "full-balance" && crossCurrency ? "full-balance" : draft.paymentMode === "advance" ? "advance" : "statement";
      const outstanding = draft.type === "transfer" && target?.type === "credit" ? creditOutstanding(target.id, draft.targetCurrency, editing) : 0;
      const rawTargetAmount = requestedMode === "full-balance" ? outstanding : (draft.targetAmount || (!crossCurrency ? draft.amount : ""));
      const targetAmount = positive(rawTargetAmount);
      if (!target || !targetAmount || !target.currencies.includes(draft.targetCurrency)) return notify(crossCurrency ? "请填写实际付款金额，并确认外币抵扣金额。" : "请选择目标账户、目标币种并填写实际金额。");
      if (draft.type === "transfer" && (target.type !== "credit" || source.type === "credit")) return notify("请使用非信用卡账户归还信用卡。");
      if (draft.type === "exchange" && target.type === "credit") return notify("购汇入账账户不能是信用卡。");
      if (requestedMode === "full-balance" && outstanding <= 0) return notify(`当前 ${draft.targetCurrency} 没有可全额抵扣的欠款。`);
      item.targetAccountId = target.id; item.targetAmount = targetAmount; item.targetCurrency = draft.targetCurrency; item.exchangeRate = round(amount / targetAmount); item.feeAmount = Math.max(0, round(draft.feeAmount)); item.feeCurrency = item.feeAmount ? (source.currencies.includes(draft.feeCurrency) ? draft.feeCurrency : item.currency) : "";
      if (draft.type === "transfer") {
        item.paymentMode = requestedMode;
        if (requestedMode === "full-balance") {
          item.statementAllocations = autoStatementAllocations(target.id, item.targetCurrency, item.date, editing, targetAmount);
        } else if (requestedMode === "advance") {
          if (targetAmount > outstanding + 0.009) return notify(`提前还款不能超过当前 ${item.targetCurrency} 欠款 ${money(outstanding, item.targetCurrency)}。`);
          item.statementAllocations = [];
        } else {
          const available = creditStatements(target.id, item.targetCurrency, { excludeTransactionId: editing });
          const byId = new Map(available.map((statement) => [statement.id, statement]));
          const allocations = (draft.statementAllocations || []).map((allocation) => ({ statementId: allocation.statementId, amount: Math.max(0, round(allocation.amount)) })).filter((allocation) => allocation.amount > 0);
          const allocated = round(allocations.reduce((sum, allocation) => sum + allocation.amount, 0));
          if (!allocations.length || Math.abs(allocated - targetAmount) > 0.009) return notify("请勾选账单，并使分配合计等于实际还款金额。");
          if (allocations.some((allocation) => !byId.has(allocation.statementId) || allocation.amount > byId.get(allocation.statementId).remaining + 0.009)) return notify("还款分配超过了所选账单的剩余应还金额。");
          item.statementAllocations = allocations;
        }
      }
    }
  ...
  function autoStatementAllocations(accountId, currency, date, excludeTransactionId, amount) {
    let remaining = Math.max(0, round(amount));
    const statements = creditStatements(accountId, currency, { excludeTransactionId, today: date }).filter((item) => item.remaining > 0).sort((a, b) => a.statementDate.localeCompare(b.statementDate) || a.dueDate.localeCompare(b.dueDate));
    return statements.map((statement) => {
      const allocation = Math.min(statement.remaining, remaining);
      remaining = round(remaining - allocation);
      return allocation > 0 ? { statementId: statement.id, amount: round(allocation) } : null;
    }).filter(Boolean);
  }
  function upsertFee(parent) { const item = { id: `${parent.id}-fee`, type: "fee", date: parent.date, amount: parent.feeAmount, currency: parent.feeCurrency, accountId: parent.accountId, targetAccountId: "", targetAmount: 0, targetCurrency: "", exchangeRate: 0, feeAmount: 0, feeCurrency: "", categoryId: feeCategory()?.id || "", note: `${parent.type === "exchange" ? "购汇" : "跨币种还款"}手续费`, installmentId: "", statementOverride: "", statementAllocations: [], paymentMode: "" }; upsert("transactions", item); }
  ...
  function creditPaymentMode(transaction) { const target = accountById(transaction.targetAccountId); if (transaction.type !== "transfer" || target?.type !== "credit") return "statement"; if (transaction.paymentMode === "full-balance") return "full-balance"; if (transaction.paymentMode === "advance" || (!transaction.paymentMode && !(transaction.statementAllocations || []).length)) return "advance"; return "statement"; }
  function creditOutstanding(accountId, currency, excludedTransactionId = "") {
    const account = accountById(accountId); if (!account) return 0;
    let amount = round(account.initialBalances?.[currency] || 0);
    state.transactions.filter((transaction) => transaction.id !== excludedTransactionId).forEach((transaction) => {
      if (transaction.accountId === accountId && transaction.currency === currency) {
        if (["transfer", "exchange", "income"].includes(transaction.type)) amount -= transaction.amount;
        else if (["expense", "fee"].includes(transaction.type)) amount += transaction.amount;
      }
      if (transaction.targetAccountId === accountId && transaction.targetCurrency === currency) {
        if (transaction.type === "transfer") amount -= transaction.targetAmount;
        else if (transaction.type === "exchange") amount += transaction.targetAmount;
      }
    });
  ...
  function creditStatements(accountId = "", currency = "", options = {}) {
    ...
    state.transactions.filter((transaction) => transaction.type === "transfer" && transaction.id !== excludedTransactionId).forEach((transaction) => {
      ...
      const mode = creditPaymentMode(transaction);
      if (mode === "advance") prepayments.push({ transaction, amount: Math.max(0, round(transaction.targetAmount)), futureOnly: false });
      if (mode === "full-balance") {
        const allocated = (transaction.statementAllocations || []).reduce((sum, allocation) => sum + Number(allocation.amount || 0), 0);
        prepayments.push({ transaction, amount: Math.max(0, round(transaction.targetAmount - allocated)), futureOnly: true });
      }
    });
    const orderedStatements = result.slice().sort((a, b) => a.statementDate.localeCompare(b.statementDate) || a.dueDate.localeCompare(b.dueDate));
    const automaticPaid = {};
    prepayments.sort((a, b) => a.transaction.date.localeCompare(b.transaction.date)).forEach(({ transaction, amount, futureOnly }) => {
      let remaining = amount;
      orderedStatements.filter((statement) => statement.accountId === transaction.targetAccountId && statement.currency === transaction.targetCurrency && (futureOnly ? statement.statementDate > transaction.date : statement.statementDate >= transaction.date)).forEach((statement) => {
        if (!remaining) return;
        const capacity = Math.max(0, round(statement.amount - (paid[statement.id] || 0) - (automaticPaid[statement.id] || 0)));
        const applied = Math.min(capacity, remaining); if (!applied) return;
        automaticPaid[statement.id] = round((automaticPaid[statement.id] || 0) + applied); remaining = round(remaining - applied);
      });
    });
    return result.map((statement) => { const paidAmount = round((paid[statement.id] || 0) + (automaticPaid[statement.id] || 0)); return { ...statement, paid: Math.min(statement.amount, paidAmount), remaining: Math.max(0, round(statement.amount - paidAmount)) }; }).sort((a, b) => a.dueDate.localeCompare(b.dueDate) || a.statementDate.localeCompare(b.statementDate));
  }
  ...
  function exportCsv() { const header = ["日期", "类型", "分类", "账户", "目标账户", "付款币种", "付款金额", "目标币种", "目标金额", "实际汇率", "手续费", "手续费币种", "分期ID", "还款方式", "账单分配", "备注"]; const lines = filteredTransactions().map((t) => [t.date, typeLabel(t.type), categoryById(t.categoryId)?.name || transferLabel(t), accountDisplayName(accountById(t.accountId)), accountDisplayName(accountById(t.targetAccountId)), t.currency, t.amount, t.targetCurrency, t.targetAmount, t.exchangeRate, t.feeAmount, t.feeCurrency, t.installmentId, t.type === "transfer" ? (t.paymentMode === "full-balance" ? "跨币种全额抵扣" : t.paymentMode === "advance" ? "提前还款" : "账单还款") : "", (t.statementAllocations || []).map((item) => `${item.statementId}:${item.amount}`).join(" | "), t.note || ""].map(csv).join(",")); download(`e-account-transactions-${stamp()}.csv`, `\ufeff${header.join(",")}\n${lines.join("\n")}`, "text/csv;charset=utf-8"); }
  ...
  function startCreditPayment(accountId, currency, statementId = "") { const source = orderedAccounts().find((account) => account.type !== "credit"); const target = accountById(accountId); if (!source || !target) return notify("请先添加付款账户和信用卡。"); const statements = creditStatements(target.id, currency).filter((item) => item.remaining > 0); const chosen = statementId ? statements.filter((item) => item.id === statementId) : statements; if (statementId && !chosen.length) return notify("这张账单已经没有剩余应还。"); const outstanding = creditOutstanding(target.id, currency); if (!chosen.length && outstanding <= 0) return notify(`当前 ${currency} 没有可提前偿还的欠款。`); draft = newDraft("transfer"); draft.accountId = source.id; draft.targetAccountId = target.id; pickerTypes.account = source.type; pickerTypes["credit-target"] = "credit"; draft.targetCurrency = currency; draft.currency = source.currencies.includes(currency) ? currency : source.currencies[0] || BASE_CURRENCY; const crossCurrency = draft.currency !== currency; draft.paymentMode = crossCurrency ? "full-balance" : chosen.length ? "statement" : "advance"; draft.fullBalanceOptOut = false; draft.statementAllocations = crossCurrency ? [] : chosen.map((item) => ({ statementId: item.id, amount: item.remaining })); draft.targetAmount = crossCurrency ? outstanding : chosen.length ? round(draft.statementAllocations.reduce((sum, item) => sum + item.amount, 0)) : ""; draft.amount = draft.currency === currency && chosen.length ? draft.targetAmount : ""; view = "record"; render(); }
