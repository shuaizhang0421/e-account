(function () {
  ...
  function repaymentFields(account) { const target = accountById(draft.targetAccountId); return `${accountPicker("付款账户", orderedAccounts().filter((a) => a.type !== "credit"), draft.accountId, "account")}${currencyPicker("付款币种", account?.currencies || [], draft.currency, "currency")}${amountDateFields("付款金额", "还款日期")}${accountPicker("还到信用卡", orderedAccounts().filter((a) => a.type === "credit"), draft.targetAccountId, "credit-target")}${currencyPicker("偿还币种", target?.currencies || [], draft.targetCurrency, "target-currency")}<label class="field"><span>信用卡实际还款金额</span><input id="draft-target-amount" type="number" step="0.01" min="0.01" value="${escapeAttr(draft.targetAmount || draft.amount)}" placeholder="外币还款可与付款金额不同"></label>${rateHint()}${feeFields(account)}${noteAndActions("保存还款")}`; }
  ...
  function accountPicker(title, accounts, activeId, mode) { return `<div><div class="panel-header"><h3>${title}</h3><span>${accounts.length} 个账户</span></div><div class="icon-grid">${accounts.map((a) => accountChoice(a, activeId, mode)).join("") || empty("暂无可用账户")}</div></div>`; }
  ...
  app.addEventListener("click", (event) => {
    ...
    if (el.dataset.targetCurrency) { syncDraftFromInputs(); draft.targetCurrency = el.dataset.targetCurrency; render(); }
  ...
  function saveTransaction() {
    ...
    const item = { id: editing || uid(), type: draft.type, date: validDate(draft.date) ? draft.date : toDate(new Date()), amount, currency: draft.currency, accountId: source.id, targetAccountId: "", targetAmount: 0, targetCurrency: "", exchangeRate: 0, feeAmount: 0, feeCurrency: "", categoryId: "", note: draft.note.slice(0, 80), installmentId: "" };
  ...
  function startCreditPayment(accountId, currency) { const source = orderedAccounts().find((account) => account.type !== "credit"); const target = accountById(accountId); if (!source || !target) return notify("请先添加付款账户和信用卡。"); draft = newDraft("transfer"); draft.accountId = source.id; draft.targetAccountId = target.id; draft.targetCurrency = currency; draft.currency = source.currencies.includes(currency) ? currency : source.currencies[0] || BASE_CURRENCY; draft.targetAmount = Math.max(0, balances()[target.id]?.[currency] || 0); draft.amount = draft.currency === currency ? draft.targetAmount : ""; view = "record"; render(); }
